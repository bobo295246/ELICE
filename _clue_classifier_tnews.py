...
def clue_evaluate(dev_jsObj):
    ...
    with torch.no_grad():
        for obj_ in tqdm.tqdm(dev_jsObj):
            ...
            if meta_mode == None:
                obj_content = fragment_content(converter.convert(obj_['sentence']), output_MaxLen, split_sent=False)
                    
            elif meta_mode == 'Single':
                obj_content = fragment_content(converter.convert(obj_['sentence']), output_MaxLen, split_sent=False) +\
                    [obj_['keywords']]
                    
            elif meta_mode == 'Multiple':
                obj_content = fragment_content(converter.convert(obj_['sentence']), output_MaxLen, split_sent=False) +\
                    obj_['keywords'].split(',')
...
def clue_inference(test_jsObj):
    ...
    with torch.no_grad():
        for obj_ in tqdm.tqdm(test_jsObj):
            ...
            pred_jsObj.append({'id': obj_['id'], 'label': label_meta['label'], 'label_desc': label_meta['label_desc']})
...
label_list = import_clue_json(Path_label)
idx = 0
for idx in range(len(label_list)):
    label_list[idx]['class'] = str(idx)
...
output_MaxLen = 128  # basic: 505
limit_fragment = 32  # for limiting the maximal amount of fragments of large-scale document
...
#model_pt_filePATH = modelPath + '\\Elice16-0518_Backbone_TinyAlbert.pt'
    ...
    if epoch % max(int((epochs/10)/4), 1) == 0:
        ...
        with torch.no_grad():
            for obj_ in tune_data_eva:
                ...
                #-------------------------------------------------------------------------#
                label_meta = get_label_metadata(obj_['label'], match='label')
                label_class = torch.tensor([int(label_meta['class'])]).unsqueeze(1).to(device)
...
save_loss(l_elice, l_task, l_acc, "tnews_base")
