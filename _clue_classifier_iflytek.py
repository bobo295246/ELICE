...
# downstream task
downstream = 'iflytek'  # afqmc, iflytek, ocnli, tnews
version = 6
...
datastep = 15200  # for limiting the maximal amount of training steps

sampling = min(int(len(tune_data_tr)), 16000)
    ...
    if epoch % max(int((epochs/10)/4), 1) == 0:
        ...
        with torch.no_grad():
            for obj_ in tune_data_eva:
                ...
                #-------------------------------------------------------------------------#
                ## task loss
                label_class = torch.tensor([int(obj_['label'])]).unsqueeze(1).to(device)
        ...
        print(evalloss_task_)
        l_acc.append(accuracy_)
save_loss(l_elice, l_task, l_acc, "iflytek_tiny")
