...
# downstream task
downstream = 'ocnli'  # afqmc, iflytek, ocnli, tnews
version = 6
...
if downstream == 'afqmc':
    ...
    Path_pred = cluePath + '\\_clue_pred\\ocnli_50k.json'
...
def clue_evaluate(dev_jsObj):
    ...
    with torch.no_grad():
        for obj_ in tqdm.tqdm(dev_jsObj):
            # sent 1 & sent 2 #----------------------------#
            label_meta = get_label_metadata(obj_['label'], match='label')
            ...
            pred_match += int((class_logit.argmax(-1) == int(label_meta['class'])).sum())
...
def clue_inference(test_jsObj):
    ...
    with torch.no_grad():
        for obj_ in tqdm.tqdm(test_jsObj):
            ...
            #-------------------------------------------------------------------------#
            label_meta = get_label_metadata(str(class_logit.argmax(-1).item()), match='class')
            pred_jsObj.append({'label': label_meta['label'], 'id': obj_['id']})
...
tune_data_tr_ = import_clue_json(Path_train)
tune_data_eva_ = import_clue_json(Path_dev)

tune_data_tr = []
for obj_ in tune_data_tr_:
    #if obj_['label'] in ['contradiction', 'entailment']:
    if obj_['label'] in ['contradiction', 'neutral', 'entailment']:
        tune_data_tr.append(obj_)

tune_data_eva = []
for obj_ in tune_data_eva_:
    #if obj_['label'] in ['contradiction', 'entailment']:
    if obj_['label'] in ['contradiction', 'neutral', 'entailment']:
        tune_data_eva.append(obj_)

label_list = [
    {'label': 'contradiction', 'class': '0'},
    {'label': 'neutral', 'class': '1'},
    {'label': 'entailment', 'class': '2'}
    ]

def get_label_metadata(item_str, match='label'):
    for jsObj in label_list:
        if item_str == jsObj[match]:
            return jsObj
...
sampling = min(int(len(tune_data_tr)), 40000)
...
#model_pt_filePATH = modelPath + '\\Elice_clue_ocnli_v4pre_Backbone.pt'
...
optimizer_tune = torch.optim.Adam([  # v2
            ...
            {'params': elice_encoder.parameters(), 'lr': 1e-4},  # , 'weight_decay': 0.001
            {'params': seq_classifier.parameters(), 'lr': 1e-4}
            ])
...
for epoch in range(1, epochs + 1):
    ...
    for training_pair in Batch_Data:
        ...
        for obj_ in training_pair:
            label_meta = get_label_metadata(obj_['label'], match='label')
            label_class.append(int(label_meta['class']))
            
            obj_content = obj_['sentence1'] + '[SEP]' + obj_['sentence2'] #if random.uniform(0,1) > 0.5 else obj_['sentence2'] + '[SEP]' + obj_['sentence1']
            obj_content = fragment_content(converter.convert(obj_content), output_MaxLen, split_sent=False)
    ...
    if epoch % max(int((epochs/10)/4), 1) == 0:
        ...
        with torch.no_grad():
            for obj_ in tune_data_eva:
                ...
                #-------------------------------------------------------------------------#
                label_meta = get_label_metadata(obj_['label'], match='label')
                label_class = torch.tensor([int(label_meta['class'])]).to(device)
                ...
                evalPred_match += int((class_logit.argmax(-1) == int(label_meta['class'])).sum())
        ...
        l_task.append(loss_task_)
        l_acc.append(accuracy_)
save_loss(l_elice, l_task, l_acc, "ocnli_base")
